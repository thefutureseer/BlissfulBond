...
import { Heart, Lock, Loader2, Eye, EyeOff, UserPlus } from "lucide-react";
...
export default function Login() {
  ...
  const users = (usersData?.users ?? []).slice().reverse();
            ...
            >
              {isLoadingUsers ? (
                <div className="col-span-2 text-center py-12">
                  ...
                  <p className="text-white/90">Loading users...</p>
                </div>
              ) : (
                <>
                  {users.map((user) => {
                    ...
                    return (
                      ...
                      >
                        <Card
                          className={`p-8 cursor-pointer bg-gradient-to-br ${gradient}/10 border-2 border-white/20 hover:border-white/40 transition-all`}
                          onClick={() => handleUserSelect(user.name)}
                          data-testid={`button-select-${user.name}`}
                        >
                          <div className="text-center space-y-4">
                            <div className={`w-20 h-20 mx-auto rounded-full bg-gradient-to-br ${gradient} flex items-center justify-center`}>
                  ...
                  >
                    <Card
                      className="p-8 cursor-pointer bg-white/10 border-2 border-white/30 hover:border-white/50 transition-all backdrop-blur-sm"
              ...
              <Card className="p-8 max-w-md mx-auto bg-white/95 backdrop-blur">
                <div className="text-center mb-6">
                  {(() => {
                    ...
                    const gradient = selectedUserData ? getGradientForUser(selectedUserData.id) : gradients[0];
                    return (
